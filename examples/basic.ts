...
const ICE_RINK = 802
const GREETINGS = ["hello!", "hey there!", "waddle on!", "hi!", "sup!", "yo!", "howdy!"]
...
console.log(`Spawned in room ${client.room} with ${client.users.size} penguins\n`)
...
client.on("add_player", ({ user }) => {
  console.log(`[JOIN] ${user.username} entered the room (${client.users.size} total)`)
})

client.on("remove_player", ({ user }) => {
  console.log(`[LEFT] Player ${user} left the room (${client.users.size} total)`)
})
...
for (const [, user] of client.users) {
  console.log(`  ${user.username} at (${user.x}, ${user.y}) color=${user.color} hat=${user.hat} head=${user.head} face=${user.face} neck=${user.neck} body=${user.body} hand=${user.hand} feet=${user.feet}`)
}
console.log()
