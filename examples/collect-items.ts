...
const EXCLUDED_IDS = new Set([
  152, 161, // Rockhopper's Eyebrows/Beard (mascot-only, cost 65535)
  380367,   // Trolling Rookie (cost 999M)
  900000, 900001, 900002, 900064, // Bait items (cost 9.9B)
])
...
async function fetchCrumbs(): Promise<{ items: Map<number, CrumbItem>; excluded: [number, CrumbItem][] }> {
  const resp = await fetch("https://cdn.cpjourney.net/assets/media/crumbs/en/crumbs.json")
  const crumbs = await resp.json() as Record<string, Record<string, CrumbItem>>
...
function waitFor<K extends string>(client: Client, event: K, timeoutMs = 10_000): Promise<unknown> {
...
console.log(`Catalog: ${catalog.size} items | Excluded: ${excludedItems.length} (max cost: ${MAX_ITEM_COST})`)
...
console.log(`Logged in as ${client.player?.username} | Coins: ${coins} | Inventory: ${inventory.size} items`)
...
console.log(`Paid items to buy: ${paid.length} (total: ${totalPaidCost.toLocaleString()} coins)`)
...
if (DRY_RUN) {
  ...
  console.log(`  ${count} items, ${(coins - budget).toLocaleString()} coins spent`)
  client.disconnect()
  process.exit(0)
}
...
async function tryAddItem(id: number, item: CrumbItem): Promise<boolean> {
  return new Promise(resolve => {
    ...
    const onError = (args: { error: number }) => {
      ...
      failed.push({ id, name: item.name, cost: item.cost, reason: `error ${args.error}` })
      resolve(false)
    }
...
if (free.length > 0) {
  ...
  for (const [id, item] of free) {
    ...
    if (ok) {
      collected++
      if (collected % 100 === 0) {
        console.log(`  [${collected}/${free.length}] last: "${item.name}" (${id})`)
      }
    }
    await client.sleep(DELAY_MS)
  }
  console.log(`Free items done: ${collected} collected, ${free.length - collected} failed`)
}
...
if (paid.length > 0) {
  ...
  for (const [id, item] of paid) {
    ...
    if (ok) {
      ...
      spent += item.cost
      if (bought % 50 === 0) {
        console.log(`  [${bought}] "${item.name}" (${item.cost}) | Balance: ${currentCoins}`)
      }
    }
    await client.sleep(DELAY_MS)
  }
  console.log(`Paid items done: ${bought} bought, ${spent.toLocaleString()} coins spent, balance: ${currentCoins}`)
}
