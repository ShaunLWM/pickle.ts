...
function waitFor<K extends string>(client: Client, event: K, timeoutMs = 15_000): Promise<unknown> {
...
console.log("Servers:", servers.map(s => `${s.name} (${s.population})`).join(", "))
...
console.log(`Logged in as: ${client.player?.username} (id: ${client.player?.id})`)
console.log(`Config: ${COINS_PER_GAME} coins, ${GAME_DURATION_MS / 1000}s game, ${BETWEEN_GAMES_MS / 1000}s pause, ${ROUNDS} rounds`)
...
for (let round = 1; round <= ROUNDS; round++) {
  ...
  const gameResult = await waitFor(client, "join_game_room") as { game: number }
  ...
  console.log(`Total coins: ${result.coins} (+${COINS_PER_GAME}), stamps: ${result.collectedStamps}/${result.totalStamps}`)
