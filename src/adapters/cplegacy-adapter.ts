...
export class CplegacyAdapter extends BaseAdapter {
  ...
  async login(options: LoginOptions | TokenLoginOptions): Promise<LoginResult> {
    ...
    const body: Record<string, unknown> = "token" in options
      ? { username: options.username, token: options.token }
      : { username: options.username, password: options.password, version: this.version }
  ...
  async connect(serverName: string, loginResult: LoginResult, options?: ConnectOptions): Promise<Socket> {
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      rawSocket.on("message", (action: string, args: Record<string, unknown>) => {
        switch (action) {
  ...
  private async postLogin(url: string, body: Record<string, unknown>): Promise<LoginResponse> {
